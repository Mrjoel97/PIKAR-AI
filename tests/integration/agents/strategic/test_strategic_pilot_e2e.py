...
"""End-to-end integration test for the strategic agent (W4)."""
...
def _todo(title: str = "Validate the productivity-tools idea") -> TodoItem:
    return TodoItem(
        id=uuid4(),
        title=title,
        description=None,
...
def _contract(*, todos: list[TodoItem] | None = None) -> TaskContract:
    return TaskContract(
        ...
        goal="Run an initiative-validation pass on the new productivity-tools idea.",
        todo_items=todos or [_todo()],
        success_criteria=[
            "TAM/SAM/SOM sized with sources.",
            "At least 3 competitors profiled with moats.",
            "Go/No-Go recommendation captured.",
        ],
        owners=[AgentID.STRAT],
...
def _complete_research() -> ResearchResult:
    return ResearchResult(
        summary="TAM $42B; 3 direct competitors profiled; 2 white-space wedges identified.",
        sources=[],
        contradictions=[],
...
def _build_strategic_pilot(user_id: UUID, persona_id: str = "startup") -> Any:
    ...
    with patch("app.agents.base_agent.PikarAgent.__init__", return_value=None):
        agent = create_strategic_agent(user_id=user_id, persona_id=persona_id)
    return agent
...
@pytest.mark.asyncio
async def test_strategic_pilot_execute_task_fires_all_four_sinks(
    db: _DBHarness,
    vault: AsyncMock,
    ...
    agent = _build_strategic_pilot(user_id, persona_id="startup")
    draft_artifact = Artifact(
        kind="doc",
        ref="vault://validation-pack",
        summary="Validation pack for productivity-tools initiative",
        payload={"markdown": "# Validation pack\n\nTAM $42B; 3 competitors profiled."},
    )
    ...
    for ev in artifact_events:
        assert ev.agent_id == AgentID.STRAT.value

    assert db.upserted_rows
    for row in db.upserted_rows:
        assert row["user_id"] == str(user_id)
        assert row["agent_id"] == AgentID.STRAT.value
        assert row["status"] == "submitted"


@pytest.mark.asyncio
async def test_strategic_pilot_ops_config_observable_post_construct() -> None:
    agent = _build_strategic_pilot(uuid4(), persona_id="startup")
    ...
    assert "strategy:*" in agent.ops.skills.allowed_ids
