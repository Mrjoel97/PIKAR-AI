...
"""End-to-end integration test for the sales agent (W4)."""
...
def _todo(title: str = "Qualify 5 inbound leads") -> TodoItem:
    return TodoItem(
        id=uuid4(),
        title=title,
        description=None,
...
def _contract(*, todos: list[TodoItem] | None = None) -> TaskContract:
    return TaskContract(
        ...
        goal="Score and prioritize this week's inbound leads.",
        todo_items=todos or [_todo()],
        success_criteria=[
            "Every lead has a BANT score.",
            "Top 3 leads have next-step recommendations.",
        ],
        owners=[AgentID.SALES],
...
def _complete_research() -> ResearchResult:
    return ResearchResult(
        summary="5 inbound leads; HubSpot context retrieved for each.",
        sources=[],
        contradictions=[],
...
def _build_sales_pilot(user_id: UUID, persona_id: str = "startup") -> Any:
    from app.agents.sales.agent import create_sales_agent

    with patch("app.agents.base_agent.PikarAgent.__init__", return_value=None):
        agent = create_sales_agent(user_id=user_id, persona_id=persona_id)
    return agent
...
@pytest.mark.asyncio
async def test_sales_pilot_execute_task_fires_all_four_sinks(
    db: _DBHarness,
    vault: AsyncMock,
    ...
    agent = _build_sales_pilot(user_id, persona_id="startup")
    draft_artifact = Artifact(
        kind="doc",
        ref="vault://lead-scores",
        summary="BANT scores",
        payload={"markdown": "# Top 3 leads\n\n..."},
    )
    ...
    for ev in artifact_events:
        assert ev.agent_id == AgentID.SALES.value

    assert db.upserted_rows
    for row in db.upserted_rows:
        assert row["user_id"] == str(user_id)
        assert row["agent_id"] == AgentID.SALES.value
        assert row["status"] == "submitted"


@pytest.mark.asyncio
async def test_sales_pilot_ops_config_observable_post_construct() -> None:
    agent = _build_sales_pilot(uuid4(), persona_id="startup")
    assert agent.ops.agent_id == "sales"
    assert agent.ops.approval.required_for_external_send is True
    assert "validation" in agent.ops.initiative.phases_owned
    assert "sales:*" in agent.ops.skills.allowed_ids
