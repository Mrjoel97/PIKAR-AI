...
"""End-to-end integration test for the compliance agent (W4)."""
...
def _todo(title: str = "GDPR Article 30 audit") -> TodoItem:
    return TodoItem(
        id=uuid4(),
        title=title,
        description=None,
...
def _contract(*, todos: list[TodoItem] | None = None) -> TaskContract:
    return TaskContract(
        ...
        goal="Complete GDPR Article 30 records of processing activities audit.",
        todo_items=todos or [_todo()],
        success_criteria=[
            "Every processing activity documented with legal basis.",
            "Risks scored on 5x5 severity-probability matrix.",
        ],
        owners=[AgentID.LEGAL],
...
def _complete_research() -> ResearchResult:
    return ResearchResult(
        summary="22 processing activities found; all categorized.",
        sources=[],
        contradictions=[],
...
def _build_compliance_pilot(user_id: UUID, persona_id: str = "startup") -> Any:
    ...
    with patch("app.agents.base_agent.PikarAgent.__init__", return_value=None):
        agent = create_compliance_agent(user_id=user_id, persona_id=persona_id)
    return agent
...
@pytest.mark.asyncio
async def test_compliance_pilot_execute_task_fires_all_four_sinks(
    db: _DBHarness,
    vault: AsyncMock,
    ...
    agent = _build_compliance_pilot(user_id, persona_id="startup")
    draft_artifact = Artifact(
        kind="doc",
        ref="vault://gdpr-audit",
        summary="GDPR Article 30 audit",
        payload={"markdown": "# Article 30\n\n22 activities documented."},
    )
    ...
    for ev in artifact_events:
        assert ev.agent_id == AgentID.LEGAL.value

    assert db.upserted_rows
    for row in db.upserted_rows:
        assert row["user_id"] == str(user_id)
        assert row["agent_id"] == AgentID.LEGAL.value
        assert row["status"] == "submitted"


@pytest.mark.asyncio
async def test_compliance_pilot_ops_config_observable_post_construct() -> None:
    agent = _build_compliance_pilot(uuid4(), persona_id="startup")
    ...
    assert "compliance:*" in agent.ops.skills.allowed_ids
