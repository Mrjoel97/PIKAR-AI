...
"""End-to-end integration test for the operations agent (W4)."""
...
def _todo(title: str = "Document onboarding SOP") -> TodoItem:
    return TodoItem(
        id=uuid4(),
        title=title,
        description=None,
...
def _contract(*, todos: list[TodoItem] | None = None) -> TaskContract:
    return TaskContract(
        ...
        goal="Author the customer onboarding SOP and roll out to support.",
        todo_items=todos or [_todo()],
        success_criteria=[
            "SOP covers Day 1 through 30-60-90.",
            "All RACI roles assigned.",
        ],
        owners=[AgentID.OPS],
...
def _complete_research() -> ResearchResult:
    return ResearchResult(
        summary="Existing onboarding patterns + 4 industry SOP templates.",
        sources=[],
        contradictions=[],
...
def _build_ops_pilot(user_id: UUID, persona_id: str = "startup") -> Any:
    ...
    with patch("app.agents.base_agent.PikarAgent.__init__", return_value=None):
        agent = create_operations_agent(user_id=user_id, persona_id=persona_id)
    return agent
...
@pytest.mark.asyncio
async def test_operations_pilot_execute_task_fires_all_four_sinks(
    db: _DBHarness,
    vault: AsyncMock,
    ...
    agent = _build_ops_pilot(user_id, persona_id="startup")
    draft_artifact = Artifact(
        kind="doc",
        ref="vault://onboarding-sop",
        summary="Onboarding SOP v1",
        payload={"markdown": "# SOP\n\n..."},
    )
    ...
    for ev in artifact_events:
        assert ev.agent_id == AgentID.OPS.value

    assert db.upserted_rows
    for row in db.upserted_rows:
        assert row["user_id"] == str(user_id)
        assert row["agent_id"] == AgentID.OPS.value
        assert row["status"] == "submitted"


@pytest.mark.asyncio
async def test_operations_pilot_ops_config_observable_post_construct() -> None:
    agent = _build_ops_pilot(uuid4(), persona_id="startup")
    ...
    assert "operations:*" in agent.ops.skills.allowed_ids
