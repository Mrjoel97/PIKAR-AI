...
"""End-to-end integration test for the customer-support agent (W4)."""
...
def _todo(title: str = "Resolve P1 ticket queue") -> TodoItem:
    return TodoItem(
        id=uuid4(),
        title=title,
        description=None,
...
def _contract(*, todos: list[TodoItem] | None = None) -> TaskContract:
    return TaskContract(
        ...
        goal="Triage and respond to the 12 open P1 support tickets.",
        todo_items=todos or [_todo()],
        success_criteria=[
            "Every P1 ticket has a drafted response.",
            "Negative-sentiment tickets are escalated to a human.",
        ],
        owners=[AgentID.SUPP],
...
def _complete_research() -> ResearchResult:
    return ResearchResult(
        summary="12 open P1 tickets; sentiment-weighted prioritization applied.",
        sources=[],
        contradictions=[],
...
def _build_cs_pilot(user_id: UUID, persona_id: str = "startup") -> Any:
    ...
    with patch("app.agents.base_agent.PikarAgent.__init__", return_value=None):
        agent = create_customer_support_agent(user_id=user_id, persona_id=persona_id)
    return agent
...
@pytest.mark.asyncio
async def test_customer_support_pilot_execute_task_fires_all_four_sinks(
    db: _DBHarness,
    vault: AsyncMock,
    ...
    agent = _build_cs_pilot(user_id, persona_id="startup")
    draft_artifact = Artifact(
        kind="doc",
        ref="vault://ticket-responses",
        summary="P1 ticket responses",
        payload={"markdown": "# Responses\n\n12 drafted, 2 escalated."},
    )
    ...
    for ev in artifact_events:
        ...
        assert ev.agent_id == AgentID.SUPP.value

    assert db.upserted_rows
    for row in db.upserted_rows:
        assert row["user_id"] == str(user_id)
        assert row["agent_id"] == AgentID.SUPP.value
        assert row["status"] == "submitted"


@pytest.mark.asyncio
async def test_customer_support_pilot_ops_config_observable_post_construct() -> None:
    agent = _build_cs_pilot(uuid4(), persona_id="startup")
    assert agent.ops.agent_id == "customer_support"
    assert agent.ops.approval.required_for_external_send is True
    assert "build" in agent.ops.initiative.phases_owned
    assert "support:*" in agent.ops.skills.allowed_ids
