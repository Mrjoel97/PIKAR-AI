...
"""Contract tests for the strategic agent (W4 migration)."""
...
_DIR = Path(__file__).resolve().parents[4] / "app" / "agents" / "strategic"
OPS_PATH = _DIR / "strategic.yaml"
...
def test_strategy_wildcard_matches_canonical_strategy_skills():
    ops = OperationsConfig.load(OPS_PATH)
    flat = _flat_skill_ids()
    hits = [s for s in flat if fnmatch.fnmatch(s, "strategy:*")]
    assert hits
    assert any(p == "strategy:*" for p in ops.skills.allowed_ids)
