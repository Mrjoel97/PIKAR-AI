...
"""Contract tests for the sales agent (W4 migration)."""
...
_SALES_DIR = Path(__file__).resolve().parents[4] / "app" / "agents" / "sales"
OPS_PATH = _SALES_DIR / "operations.yaml"
INSTRUCTIONS_PATH = _SALES_DIR / "instructions.md"
...
def test_sales_wildcard_matches_canonical_sales_skills():
    ops = OperationsConfig.load(OPS_PATH)
    flat = _flat_skill_ids()
    sales_hits = [s for s in flat if fnmatch.fnmatch(s, "sales:*")]
    assert sales_hits, (
        "no skills with category=sales; the sales:* wildcard would resolve to nothing"
    )
    assert any(p == "sales:*" for p in ops.skills.allowed_ids)
