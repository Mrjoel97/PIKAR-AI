...
"""Contract tests for the compliance agent (W4 migration)."""
...
_DIR = Path(__file__).resolve().parents[4] / "app" / "agents" / "compliance"
OPS_PATH = _DIR / "operations.yaml"
...
def test_compliance_wildcard_matches_canonical_compliance_skills():
    ops = OperationsConfig.load(OPS_PATH)
    flat = _flat_skill_ids()
    hits = [s for s in flat if fnmatch.fnmatch(s, "compliance:*")]
    assert hits
    assert any(p == "compliance:*" for p in ops.skills.allowed_ids)
