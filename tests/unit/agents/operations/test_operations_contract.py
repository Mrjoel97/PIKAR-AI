...
"""Contract tests for the operations agent (W4 migration)."""
...
_DIR = Path(__file__).resolve().parents[4] / "app" / "agents" / "operations"
...
def test_operations_wildcard_matches_canonical_operations_skills():
    ops = OperationsConfig.load(OPS_PATH)
    flat = _flat_skill_ids()
    hits = [s for s in flat if fnmatch.fnmatch(s, "operations:*")]
    assert hits
    assert any(p == "operations:*" for p in ops.skills.allowed_ids)
