...
**Plans:** 5 plans (5 waves — sequential per plan-checker iteration 1; both Plans 02 and 03 mutate `app/routers/workflows.py` so they must run sequentially)
...
- [ ] 110-04-frontend-editable-canvas-PLAN.md — Wave 4 (depends 02 + 03) — drag-palette + properties drawer + Zod schemas + client validation + Save flow with comment modal + seed-copy redirect
- [ ] 110-05-frontend-versioning-conflict-PLAN.md — Wave 5 (depends 02 + 04) — VersionSelector + HistoryPane + ConflictModal (three-button) + revertTemplate service + save→reload round-trip integration test
