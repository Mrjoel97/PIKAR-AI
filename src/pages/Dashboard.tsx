...
function TierSidebar({ tier, onNavigate }: { tier: string; onNavigate: (path: string) => void }) {
  // Normalize tier
  const t = (tier || "solopreneur").toLowerCase();

  // Define menu by tier using only existing routes/pages
  type Item = { label: string; icon: React.ElementType; path: string };
  const base: Array<Item> = [
    { label: "Dashboard", icon: BarChart3, path: "/dashboard" },
    { label: "Agents", icon: Bot, path: "/agents" },
    { label: "Workflows", icon: Brain, path: "/workflows" },
    { label: "Business Settings", icon: Settings, path: "/business" },
  ];
  const startupExtra: Array<Item> = [
    { label: "Analytics", icon: BarChart3, path: "/analytics" },
    { label: "Initiatives", icon: TrendingUp, path: "/initiatives" },
  ];
  const smeExtra: Array<Item> = startupExtra;
  const enterpriseExtra: Array<Item> = startupExtra;

  const menu: Array<Item> =
    t === "solopreneur"
      ? base
      : t === "startup"
      ? [...base.slice(0, 3), ...startupExtra, base[3]]
      : t === "sme"
      ? [...base.slice(0, 3), ...smeExtra, base[3]]
      : t === "enterprise"
      ? [...base.slice(0, 3), ...enterpriseExtra, base[3]]
      : base;
  ...
  return (
    <Card className="sticky top-6 h-fit">
      <CardHeader className="pb-3">
        ...
        <CardDescription>Tier-specific access</CardDescription>
      </CardHeader>
        ...
        <nav className="flex flex-col">
          {menu.map((it) => (
            <Button
              ...
              variant="ghost"
              className={cn(
                "justify-start gap-2 rounded-md px-3 py-2",
              )}
...
export default function Dashboard() {
  ...
  const tier = (currentBusiness as any).tier || "solopreneur";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-6">
          {/* Tier-specific sidebar */}
          <aside className="order-2 lg:order-1">
            <TierSidebar tier={tier} onNavigate={navigate} />
          </aside>
