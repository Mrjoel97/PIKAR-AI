...
class CampaignService(BaseService):
    ...
    async def get_campaign(self, campaign_id: str, user_id: str | None = None) -> dict:
        ...
        effective_user_id = user_id or get_current_user_id()
        client = self.client if self.is_authenticated else AdminService().client
        query = client.table(self._table_name).select("*").eq("id", campaign_id)
    ...
    async def update_campaign(
        ...
        effective_user_id = user_id or get_current_user_id()
        client = self.client if self.is_authenticated else AdminService().client
        query = client.table(self._table_name).update(update_data).eq("id", campaign_id)
        if not self.is_authenticated and effective_user_id:
            query = query.eq("user_id", effective_user_id)
    ...
    async def delete_campaign(
        ...
        effective_user_id = user_id or get_current_user_id()
        client = self.client if self.is_authenticated else AdminService().client
        query = client.table(self._table_name).delete().eq("id", campaign_id)
        if not self.is_authenticated and effective_user_id:
            query = query.eq("user_id", effective_user_id)
        response = await execute_async(query)
    ...
    async def record_metrics(
        ...
        return await self.update_campaign(
            campaign_id, metrics=metrics, user_id=user_id or get_current_user_id()
        )
