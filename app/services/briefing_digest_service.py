...
async def format_digest_html(
    ...
    for item in display_items:
        priority = item.get("priority", "normal")
        _label, bg, fg = _PRIORITY_STYLES.get(
            priority, ("NORMAL", "#6366f1", "#ffffff")
        )
        badge_html = (
...
def _format_plain_text(
    triage_items: list[dict], user_name: str, digest_date: date
) -> str:
...
async def send_digest_email(user_id: str) -> dict[str, Any]:
    ...
    if not items:
        logger.debug(
            "No triage items for user %s on %s — skipping digest", user_id, today_str
        )
