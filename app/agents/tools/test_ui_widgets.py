...
def _assert_valid_widget(result: dict, expected_type: str):
    ...
    assert result.get("type") == expected_type, f"Expected type '{expected_type}', got '{result.get('type')}'"
    assert isinstance(result.get("data"), dict), "Widget must have a 'data' dict"
    assert result["type"] in VALID_WIDGET_TYPES, f"Widget type '{result['type']}' not in valid types"
...
class TestProductLaunchWidget:
    ...
    def test_returns_valid_widget(self):
        result = create_product_launch_widget(milestones=[
...
class TestMorningBriefingWidget:
    ...
    def test_data_fields(self):
        result = create_morning_briefing_widget(
            greeting="Hello",
            pending_approvals=[{"id": "1", "action_type": "deploy", "created_at": "2024-01-01", "token": "abc"}],
            online_agents=3,
            system_status="healthy"
        )
...
class TestBoardroomWidget:
    ...
    def test_returns_valid_widget(self):
        result = create_boardroom_widget(
            topic="Q3 Strategy",
            transcript=[{"speaker": "CEO", "content": "We need to focus...", "sentiment": "positive"}],
            verdict="Approved"
        )
...
class TestSuggestedWorkflowsWidget:
    ...
    def test_returns_valid_widget(self):
        result = create_suggested_workflows_widget(suggestions=[
            {"id": "s1", "pattern_description": "Weekly review", "suggested_goal": "...", "suggested_context": "...", "status": "new"}
        ])
...
class TestCalendarWidget:
    ...
    def test_returns_valid_widget(self):
        result = create_calendar_widget(
            events=[{"id": "e1", "title": "Meeting", "start": "2024-01-01", "end": "2024-01-01"}]
        )
...
class TestUIWidgetToolsExport:
    ...
    def test_all_widget_types_covered(self):
        ...
        produced_types.add(create_morning_briefing_widget(greeting="", pending_approvals=[], online_agents=0, system_status="ok")["type"])
        produced_types.add(create_boardroom_widget(topic="", transcript=[], verdict="")["type"])
