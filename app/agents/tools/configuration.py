...
"""
    ...
    benefits_text += recommendations.get(
        tool_id, "This tool can enhance your AI assistant's capabilities."
    )
...
async def save_user_api_key(user_id: str, tool_id: str, api_key: str) -> dict[str, Any]:
    ...
    for tool_id, guide in MCP_TOOL_GUIDES.items():
        ...
        if env_value and len(env_value) > 5:
            ...
            unconfigured_tools.append(
                {"id": tool_id, "name": guide["name"], "benefit": guide["use_cases"][0]}
            )
...
"""
    ...
    for tool_id, guide in MCP_TOOL_GUIDES.items():
        env_value = os.environ.get(guide["env_var"])
        status = (
            "✅ Configured" if (env_value and len(env_value) > 5) else "⚪ Not set up"
        )
...
def recommend_tools_for_goal(goal: str) -> dict[str, Any]:
    ...
    if any(
        word in goal_lower
        for word in ["website", "scrape", "content", "extract", "article", "competitor"]
    ):
        recommendations.append(
            {
                "tool_id": "firecrawl",
    ...
    if any(
        word in goal_lower
        for word in ["landing", "page", "campaign", "launch", "marketing", "lead"]
    ):
        recommendations.append(
            {
                "tool_id": "stitch",
    ...
    if any(
        word in goal_lower
        for word in ["email", "send", "notify", "newsletter", "outreach"]
    ):
        recommendations.append(
            {
                "tool_id": "resend",
    ...
    if any(
        word in goal_lower
        for word in ["crm", "customer", "lead", "sales", "track", "contact", "deal"]
    ):
        recommendations.append(
            {
                "tool_id": "hubspot",
