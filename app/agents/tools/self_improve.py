...
def _create_report_interaction(agent_id: AgentID) -> Callable:
    ...
    async def report_interaction(
        ...
        try:
            ...
            updated = False
            if session_id:
                updated = await il.update_latest_interaction(
            ...
            if not updated:
                row_id = await il.log_interaction(
                    agent_id=agent_id.value,
...
def _create_check_performance(agent_id: AgentID) -> Callable:
    ...
    @agent_tool
    async def check_my_performance(
        days: int = 7,
    ) -> dict[str, Any]:
        ...
        try:
            il = _get_logger()
            stats = await il.get_interaction_stats(
                agent_id=agent_id.value,
                days=days,
            )
            if not stats:
...
def _create_get_improvement_suggestions(agent_id: AgentID) -> Callable:
    ...
    async def get_improvement_suggestions(
        ...
        try:
            engine = _get_engine()
            suggestions = await engine.get_pending_actions(
                agent_id=agent_id.value,
                limit=limit,
            )
            if not suggestions:
