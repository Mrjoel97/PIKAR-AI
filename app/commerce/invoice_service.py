...
class InvoiceService:
    ...
    async def generate_invoice_pdf(self, invoice_data: Dict[str, Any]) -> bytes:
        ...
        c.drawString(50, height - 80, f"Invoice #: {invoice_data.get('invoice_number', 'DRAFT')}")
        c.drawString(50, height - 100, f"Date: {invoice_data.get('date', datetime.date.today().isoformat())}")
    ...
    async def create_invoice_record(self, user_id: str, data: Dict[str, Any]) -> str:
        ...
        invoice_record = {
            ...
            "total_amount": data.get("total_amount", 0.0), # Schema doesn't have total_amount on invoice directly, it's on Order
            # Wait, schema check:
