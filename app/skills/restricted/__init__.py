...
# Configuration
ALLOW_SECURITY_SKILLS = (
    os.environ.get("ALLOW_SECURITY_SKILLS", "false").lower() == "true"
)
SECURITY_SKILLS_AUDIT_LOG = (
    os.environ.get("SECURITY_SKILLS_AUDIT_LOG", "true").lower() == "true"
)
...
def is_skill_restricted(skill_name: str, skill_category: str | None = None) -> bool:
...
def check_skill_access(skill_name: str, user_id: str | None = None) -> bool:
...
def audit_skill_usage(
    skill_name: str, user_id: str | None, action: str, success: bool
) -> None:
...
def filter_restricted_skills(skills: list[dict]) -> list[dict]:
    ...
    return [
        skill
        for skill in skills
        if not is_skill_restricted(skill.get("name", ""), skill.get("category"))
    ]
