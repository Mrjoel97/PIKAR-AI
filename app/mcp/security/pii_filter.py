...
class PIIFilter:
    ...
    PATTERNS: dict[str, tuple[re.Pattern, str]] = {
        ...
        "date_of_birth": (
            re.compile(
                r"\b(?:DOB|D\.O\.B\.?|born|birthday)[:\s]*[0-9]{1,2}[/\-][0-9]{1,2}[/\-][0-9]{2,4}\b",
                re.IGNORECASE,
            ),
            "[DOB_REDACTED]",
        ),
    ...
    def __init__(
        self, custom_patterns: dict[str, tuple[re.Pattern, str]] | None = None
    ):
...
def sanitize_query(query: str, context_names: list[str] | None = None) -> str:
