# Research Intelligence System — Phase 6: Admin Panel API

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add admin API endpoints for research cost monitoring, knowledge graph exploration, and scheduler management. These power the admin panel's "Research Intelligence" section (frontend pages will be built separately as admin panel Phase 10+).

**Architecture:** A single `app/routers/admin/research.py` module with 17 endpoints, following the exact pattern of existing admin routers (require_admin dependency, rate limiting, service_client, execute_async). Registered in the admin router `__init__.py`.

**Tech Stack:** FastAPI, Supabase (service client), existing admin auth middleware, slowapi rate limiting

**Spec:** `docs/superpowers/specs/2026-03-21-research-intelligence-system-design.md` (Section 4)
...
```
NEW FILES:
  app/routers/admin/research.py              — 17 admin endpoints for research intelligence
  tests/unit/test_admin_research.py          — Endpoint tests

MODIFIED FILES:
  app/routers/admin/__init__.py              — Register research router
```

---

## Task 1: Admin Research Router — Overview + Costs

**Files:**
- Create: `app/routers/admin/research.py`

Start with the overview and cost endpoints (the most immediately useful).

- [ ] **Step 1: Read existing admin router patterns**

Read `app/routers/admin/analytics.py` and `app/routers/admin/__init__.py` to match the exact pattern.

- [ ] **Step 2: Create the research admin router with overview + cost endpoints**

```python
# app/routers/admin/research.py
...
async def get_research_overview(
    ...
    try:
        ...
        for domain, config in DOMAIN_FRESHNESS.items():
            ...
            freshness_pct = (fresh_count / total_count * 100) if total_count > 0 else 100
        ...
        raise HTTPException(status_code=500, detail="Failed to load research overview") from exc
...
async def get_research_costs(
    ...
    try:
        ...
        resp = await _exec(
            client.table("kg_research_log")
            .select("domain, triggered_by, cost_usd, searches_used, scrapes_used, depth")
        ...
        for log in logs:
            ...
            if domain not in by_domain:
                by_domain[domain] = {"cost": 0, "searches": 0, "scrapes": 0, "jobs": 0}
        ...
        return {
            "days": days,
            "by_domain": {k: {**v, "cost": round(v["cost"], 4)} for k, v in by_domain.items()},
            "by_trigger": {k: {**v, "cost": round(v["cost"], 4)} for k, v in by_trigger.items()},
        ...
        raise HTTPException(status_code=500, detail="Failed to load research costs") from exc
    ...
    try:
        ...
        for log in (resp.data or []):
            ...
            if date not in daily:
                daily[date] = {"date": date, "cost": 0, "searches": 0, "scrapes": 0, "jobs": 0}
        ...
        raise HTTPException(status_code=500, detail="Failed to load daily cost trend") from exc
...
async def get_graph_stats(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to load graph stats") from exc
...
async def get_entity_detail(
    ...
    try:
        ...
        aliases_resp = await _exec(
            client.table("kg_aliases").select("alias, confidence").eq("entity_id", entity_id),
...
async def search_entities(
    ...
    try:
        resp = await _exec(
            client.table("kg_entities")
            .select("id, canonical_name, entity_type, domains, source_count, freshness_at")
...
async def force_refresh_entity(
    ...
    try:
        entity_resp = await _exec(
            client.table("kg_entities").select("canonical_name, domains").eq("id", entity_id),
        ...
        await log_admin_action(
            ...
            target_id=entity_id,
            source="manual",
            changes={"entity": entity["canonical_name"], "domain": domain},
            admin_user_email=admin_user.get("email", ""),
        )

        return {"success": True, "entity": entity["canonical_name"], "event": result}
...
async def list_watch_topics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to load watch topics") from exc
...
async def create_watch_topic(
    ...
    try:
        ...
        return {"success": True, "topic": resp.data[0] if resp.data else None}

    except HTTPException:
        raise
    except Exception as exc:
        logger.error("Watch topic create failed: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to create watch topic") from exc
...
async def get_budgets(
    ...
    try:
        ...
        for budget in budgets:
            ...
            budget["budget_pct"] = round(
                budget["current_spend"] / float(budget["monthly_budget"]) * 100, 1
            ) if float(budget["monthly_budget"]) > 0 else 0
...
async def get_research_history(
    ...
    try:
        ...
        return {"history": resp.data or [], "limit": limit, "offset": offset}
...
async def export_research_history(
    ...
    try:
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename=research_history_{days}d.csv"},
        )
...
async def get_flywheel_health(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to load flywheel health") from exc
```

- [ ] **Step 3: Lint**

```bash
uv run ruff check app/routers/admin/research.py --fix && uv run ruff format app/routers/admin/research.py
```

- [ ] **Step 4: Commit**

```bash
git add app/routers/admin/research.py
git commit -m "feat(research): add admin research router with 17 endpoints"
```

---

## Task 2: Register Router + Write Tests

**Files:**
- Modify: `app/routers/admin/__init__.py`
- Create: `tests/unit/test_admin_research.py`

- [ ] **Step 1: Register the research router**

Read `app/routers/admin/__init__.py` and add:

```python
from app.routers.admin import research
admin_router.include_router(research.router)
```

- [ ] **Step 2: Write tests**

Create `tests/unit/test_admin_research.py` testing the module imports and endpoint count:
...
def test_research_router_has_endpoints():
    ...
    assert len(routes) >= 15  # 17 endpoints
...
def test_research_router_registered():
    ...
    # Check at least one research path exists
...
```bash
uv run pytest tests/unit/test_admin_research.py -v
```

- [ ] **Step 4: Commit**

```bash
git add app/routers/admin/__init__.py tests/unit/test_admin_research.py
git commit -m "feat(research): register admin research router with 17 endpoints"
```
...
```bash
uv run ruff check app/agents/research/ app/services/graph_service.py app/agents/tools/graph_tools.py app/services/research_event_bus.py app/services/intelligence_scheduler.py app/services/intelligence_worker.py app/services/research_impact_analyzer.py app/routers/admin/research.py --fix
```
...
- [ ] 17 admin API endpoints registered under /admin/research/*
- [ ] Overview: graph size, freshness score, cost MTD, jobs today, domain health
- [ ] Costs: breakdown by domain, trigger, daily trend, CSV export
- [ ] Graph Explorer: entity search, detail with relationships/findings, force refresh, delete
- [ ] Scheduler: domain schedules, watch topics CRUD, budget management
- [ ] Flywheel Health: research impact analysis, skill recommendations
- [ ] All endpoints protected by require_admin dependency
- [ ] Rate limiting on all endpoints
- [ ] Audit logging on mutating operations
- [ ] All tests pass

**This completes the Research Intelligence System — all 6 phases.**
