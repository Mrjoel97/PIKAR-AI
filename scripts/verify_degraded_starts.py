...
async def main():
    try:
        ...
        # "degraded-simulation-prone" label
        target_workflows = report["workflow_names_by_label"].get("degraded-simulation-prone", [])
        ...
        for name in target_workflows:
            ...
            try:
                ...
                if "error" in result:
                    # If it fails, log it. These are expected to be "degraded" so maybe some fail?
                    # But readiness says "ready".
